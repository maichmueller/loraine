...
class Effect {
  public:
   ...
   void set_assoc_card(sptr<Card> card_ptr) { m_assoc_card = std::move(card_ptr); }
   void set_effect(
   ...
   Effect(
      std::function< void(State&, const events::VariantEvent&) > effect, sptr<Card> card_ptr)
       : m_effect(std::move(effect)), m_is_null(false), m_assoc_card(std::move(card_ptr))
   {
   }
