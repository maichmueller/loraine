...
void Logic::damage_nexus(size_t amount, Player player)
{
   m_state->nexus_health(player) -= amount;
}
void Logic::heal_nexus(size_t amount, Player player)
{
   m_state->nexus_health(player) = std::min(
      m_state->nexus_health(player) + static_cast< long >(amount),
      static_cast< long >(m_state->config().START_NEXUS_HEALTH));
}
bool Logic::round_ended() const
{
   return m_state->pass_flag(Player::BLUE) && m_state->pass_flag(Player::RED);
}

bool Logic::pass()
{
   m_state->pass_flag(m_state->active_player()) = true;
   return round_ended();
}
void Logic::reset_pass(Player player)
{
   m_state->pass_flag(player) = false;
}
auto Logic::is_enlightened(Player player) const
{
   return m_state->managems(player) == m_state->config().MAX_MANA;
}

void Logic::shuffle_card_into_deck(const sptr< Card >& card, Player player)
{
...
void Logic::_move_units(const std::vector< size_t >& positions, Player player, bool to_bf)
{
   auto min_target_pos = m_state->board()->count_units(player, not to_bf);
   auto& battlefield = m_state->board()->get_battlefield(player);
   auto& camp = m_state->board()->get_camp(player);
   if(to_bf) {
...
void Logic::_move_units_opp(const std::map< size_t, size_t >& positions, Player player, bool to_bf)
{
   auto& battlefield = m_state->board()->get_battlefield(player);
   auto& camp = m_state->board()->get_camp(player);
   ...
   if(to_bf) {
      auto& bf_opp = m_state->board()->get_battlefield(Player(1 - player));
...
bool Logic::_move_spell(const sptr< Spell >& spell, bool to_stack)
{
...
bool Logic::run_game()
{
   ...
   Player starting_player = Player(std::uniform_int_distribution< size_t >(0, 1)(rng::engine()));
...
void Logic::_activate_battlemode(Player attack_player)
{
   m_state->set_battle_mode(true);
   m_state->attacker(attack_player);
   auto& bf = m_state->board()->get_battlefield(attack_player);
   ...
   if(! m_state->token_scout(attack_player)) {
      for(auto opt_unit : m_state->board()->get_battlefield(attack_player)) {
...
void Logic::_resolve_battle()
{
   ...
   auto& battlefield_att = m_state->board()->get_battlefield(attacker);
   auto& battlefield_def = m_state->board()->get_battlefield(defender);
...
void Logic::kill_unit(Player killer, const sptr< Unit >& killed_unit, const sptr< Card >& cause)
{
   killed_unit->kill();
...
void Logic::nexus_strike(Player attacked_nexus, const sptr< Unit >& striking_unit)
{
...
std::vector< sptr< Card > > Logic::_draw_n_cards(Player player, int n)
{
...
void Logic::_end_round()
{
   ...
   auto end_round_procedure = [&](Player player) {
      ...
      for(auto& unit : m_state->board()->get_camp(player)) {
...
void Logic::give_managems(Player player, long amount)
{
   m_state->incr_managems(player, amount);
   _check_enlightenment(player);
}

void Logic::_check_enlightenment(Player player)
{
   if(m_state->is_enlightened(player)) {
      _trigger_event(events::EnlightenmentEvent(player));
   }
}
void Logic::retreat_to_camp(Player player)
{
   process_camp_queue(player);

   size_t camp_units_count = m_state->board()->count_units(player, true);
   size_t bf_units_count = m_state->board()->count_units(player, false);
   // obliterate any battling units which won't find space on the board
   auto& bf = m_state->board()->get_battlefield(player);
...
void Logic::play(const sptr< Card >& card, std::optional< size_t > replaces)
{
   ...
   if(replaces.has_value()) {
      ...
      camp.at(m_state->board()->count_units(player, true) - 1) = card;
   }
...
void Logic::_play_event_triggers(const sptr< Card >& card, const Player& player)
{
...
void Logic::process_camp_queue(Player player)
{
   ...
   size_t camp_units_count = m_state->board()->count_units(player, true);
   size_t bf_units_count = m_state->board()->count_units(
      player, false, [](const sptr< Card >& unit) {
         return not unit->has_keyword(Keyword::EPHEMERAL);
      });
   auto& queue = m_state->board()->get_camp_queue(player);
...
void Logic::spend_mana(Player player, size_t cost, bool spell_mana)
{
...
void Logic::draw_card(Player player)
{
   auto deck = m_state->deck(player);
...
void Logic::summon_to_battlefield(const sptr< Unit >& unit)
{
   Player player = unit->mutables().owner;
   auto bf = m_state->board()->get_battlefield(player);
   auto curr_unit_count = m_state->board()->count_units(player, false);
...
std::vector< Target > Logic::filter_targets_bf(
...
void Logic::heal(Player player, const sptr< Unit >& unit, long amount)
{
   if(amount > 0) {
      unit->heal(amount);
...
bool Logic::check_daybreak(Player player) const
{
...
bool Logic::check_nightfall(Player player) const
{
   const auto& history = m_state->history()->at(player).at(m_state->round());
...
void Logic::_remove(const sptr< Card >& card)
{
   ...
   if(auto loc = card->mutables().location; loc == Location::CAMP) {
      algo::remove_element(m_state->board()->get_camp(card->mutables().owner), card);
   } else if(loc == Location::BATTLEFIELD) {
      algo::remove_element(m_state->board()->get_battlefield(card->mutables().owner), card);
