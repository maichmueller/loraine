...
bool Logic::_move_spell(const sptr< Spell >& spell, bool to_stack)
{
   ...
   if(to_stack) {
      ...
      for(auto& effect : cast_effects) {
         if(not effect.target(*m_state, *get_agent(team), team)) {
            // the agent has cancelled playing the floating, we thus return handling to the action
            // method
            return false;
         }
      }
