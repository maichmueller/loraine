...
void GameMode::_move_units(const std::vector< size_t >& positions, Team team, bool to_bf)
{
   auto min_target_pos = m_state->m_board->count_units(team, not to_bf);
   auto& battlefield = m_state->m_board->battlefield(team);
   auto& camp = m_state->m_board->camp(team);
   if(to_bf) {
...
void GameMode::_move_units_opp(const std::map< size_t, size_t >& positions, Team team, bool to_bf)
{
   auto& battlefield = m_state->m_board->battlefield(team);
   auto& camp = m_state->m_board->camp(team);
   ...
   if(to_bf) {
      auto& bf_opp = m_state->m_board->battlefield(Team(1 - team));
...
bool GameMode::_move_spell(const sptr< Spell >& spell, bool to_stack)
{
   ...
   if(to_stack) {
      ...
      for(auto& effect : cast_effects) {
         if(not effect.target(*m_state, *agent(team), team)) {
...
bool GameMode::_do_action(const sptr< Action >& action)
{
      ...
      switch(action_type) {
         ...
         case MOVE_SPELL: {
            ...
            if(not _move_spell(spell, cast_action->to_stack())) {
               // the spell was supposed to be moved to the stack, but the team cancelled
...
void GameMode::_activate_battlemode(Team attack_team)
{
   ...
   auto& bf = m_state->m_board->battlefield(attack_team);
   ...
   auto check_supp_effects = [&](const sptr< Card >& unit, const sptr< Card >& next_unit) {
      if(unit->has_keyword(Keyword::SUPPORT)) {
         _trigger_event(events::SupportEvent(attack_team, unit, Target(next_unit)));
      }
   };
   ...
   if(! m_state->token_scout(attack_team)) {
      for(auto opt_unit : m_state->m_board->battlefield(attack_team)) {
...
void GameMode::_deactivate_battlemode()
{
   m_state->set_battle_mode(false);
   m_state->reset_attacker();
}
void GameMode::_resolve_spell_stack(bool burst)
{
...
void GameMode::_resolve_battle()
{
   m_state->set_battle_resolution_mode(true);
   auto attacker = m_state->attacker().value();
   auto defender = Team(1 - attacker);
   auto& battlefield_att = m_state->m_board->battlefield(attacker);
   auto& battlefield_def = m_state->m_board->battlefield(defender);
...
void GameMode::strike(const sptr< Unit >& unit_att, sptr< Unit >& unit_def)
{
...
void GameMode::deal_damage_to_unit(
...
void GameMode::kill_unit(Team killer, const sptr< Unit >& killed_unit, const sptr< Card >& cause)
{
   killed_unit->kill();
...
void GameMode::nexus_strike(Team attacked_nexus, const sptr< Unit >& striking_unit)
{
...
std::vector< sptr< Card > > GameMode::_draw_n_cards(Team team, int n)
{
...
void GameMode::_end_round()
{
   ...
   auto end_round_procedure = [&](Team team) {
      ...
      for(auto& unit : m_state->m_board->camp(team)) {
...
void GameMode::_start_round()
{
   ...
   _trigger_event(
      events::RoundStartEvent(m_state->active_team(), std::make_shared< long >(round)));
   for(int team = BLUE; team != RED; ++team) {
      incr_managems(Team(team));
   }
...
void GameMode::incr_managems(Team team, long amount)
{
   m_state->incr_managems(team, amount);
   _check_enlightenment(team);
}

void GameMode::_check_enlightenment(Team team)
{
   if(m_state->is_enlightened(team)) {
      _trigger_event(events::EnlightenmentEvent(team));
   }
}
void GameMode::retreat_to_camp(Team team)
{
   process_camp_queue(team);

   size_t camp_units_count = m_state->m_board->count_units(team, true);
   size_t bf_units_count = m_state->m_board->count_units(team, false);
   // obliterate any battling units which won't find space on the board
   auto& bf = m_state->m_board->battlefield(team);
...
void GameMode::play(const sptr< Card >& card, std::optional< size_t > replaces)
{
   ...
   if(replaces.has_value()) {
      ...
      camp.at(m_state->m_board->count_units(team, true) - 1) = card;
   }
...
void GameMode::cast(const sptr< Spell >& spell)
{
...
void GameMode::process_camp_queue(Team team)
{
   ...
   size_t bf_units_count = m_state->m_board->count_units(
      team, false, [](const sptr< Card >& unit) {
         return not unit->has_keyword(Keyword::EPHEMERAL);
      });
   auto& queue = m_state->m_board->camp_queue(team);
...
void GameMode::spend_mana(Team team, size_t cost, bool floating_mana)
{
...
void GameMode::draw_card(Team team)
{
   auto deck = m_state->deck(team);
...
void GameMode::summon(const sptr< Unit >& unit)
{
...
void GameMode::summon_to_battlefield(const sptr< Unit >& unit)
{
   Team team = unit->mutables().owner;
   auto bf = m_state->m_board->battlefield(team);
   auto curr_unit_count = m_state->m_board->count_units(team, false);
...
void GameMode::summon_exact_copy(const sptr< Unit >& unit)
{
   Team team = unit->mutables().owner;
...
std::vector< Target > GameMode::filter_targets_bf(
...
std::vector< Target > GameMode::filter_targets_camp(
   const Filter& filter, std::optional< Team > opt_team)
{
   std::vector< Target > targets;
...
std::vector< Target > GameMode::filter_targets_hand(
   const Filter& filter, std::optional< Team > opt_team)
{
   std::vector< Target > targets;
...
std::vector< Target > GameMode::filter_targets_deck(
   const Filter& filter, std::optional< Team > opt_team)
{
   std::vector< Target > targets;
...
std::vector< Target > GameMode::filter_targets_board(
   const Filter& filter, std::optional< Team > opt_team)
{
   std::vector< Target > targets;
...
std::vector< Target > GameMode::filter_targets_everywhere(
   const Filter& filter, std::optional< Team > opt_team)
{
   std::vector< Target > targets;
...
void GameMode::heal(Team team, const sptr< Unit >& unit, long amount)
{
   if(amount > 0) {
      unit->heal(amount);
...
bool GameMode::check_daybreak(Team team) const
{
   ...
   return std::find_if(
             ...
             [](const sptr< Action >& action) {
                return action->action_type() == ActionType::PLAY;
             })
          == history.end();
}
bool GameMode::check_nightfall(Team team) const
{
   const auto& history = m_state->history()->at(team).at(m_state->round());
